...
function AboutToolsRow({ tools }) {
  ...
  return (
    <div className="about-tools-row mt-5 xl:mt-[50px]">
        ...
        {loopTools.map((item, index) => (
          ...
          >
            <div className="flex items-center gap-2 sm:w-full sm:items-start sm:justify-between sm:gap-3">
              ...
              <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[8px] p-1.5 sm:h-8 sm:w-8 sm:rounded-[8px] xl:h-[29px] xl:w-[29px] xl:rounded-[6px] xl:p-0 2xl:h-12 2xl:w-12 min-[1920px]:h-[56px] min-[1920px]:w-[56px]">
                <img src={item.icon} alt="" aria-hidden="true" className="h-full w-full object-contain" />
              </span>
            </div>
            <p className="hidden sm:block mt-3 text-xs font-light leading-[1.4] text-brand-ink/80 sm:text-sm xl:text-[15px] xl:leading-[1.4] xl:tracking-normal min-[1920px]:mt-[8px] min-[1920px]:text-[14px]">
              {item.description}
            </p>
          </article>
        ))}
